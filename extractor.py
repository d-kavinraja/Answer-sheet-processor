...
# Corrected imports: Added st_info, time, and datetime
...
class AnswerSheetExtractor:
    def __init__(self, yolo_improved_path, yolo_fallback_path, register_crnn_path, subject_crnn_path):
        # Correctly uses st_info which is now imported
        try:
    ...
    def _detect_regions(self, image_path, model):
        ...
        register_regions, subject_regions, overlay = [], [], image.copy()
        for box in detections:
            ...
            if "Register" in label:
                register_regions.append((cropped_region, float(box.conf[0])))
            else:
                subject_regions.append((cropped_region, float(box.conf[0])))
    ...
    def process_answer_sheet(self, image_path):
        ...
        if reg:
            best_reg_crop, _ = max(reg, key=lambda item: item[1])
        ...
        if subj:
            best_subj_crop, _ = max(subj, key=lambda item: item[1])
        ...
        if not results:
            st_warning("No scannable regions were detected.")

        overlay_path = os.path.join("results", f"overlay_{uuid.uuid4().hex}.jpg")
        cv2.imwrite(overlay_path, overlay)
        processing_time_val = time.time() - start_time
        history_item = {
        ...
        return results, reg_path, subj_path, overlay_path, processing_time_val, history_item
