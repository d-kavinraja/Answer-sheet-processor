...
def display_header():
    with st.container():
        ...
        with col1:
             ...
             st.markdown('<div style="font-size: 60px; text-align: center;">📝</div>', unsafe_allow_html=True) # Emoji as icon
        with col2:
...
def main():
    ...
    if selected_tab == "Scan":
        ...
        with cols[0]:
             if st.button("⬆️ Upload Image", key="upload_image_btn", use_container_width=True):
                 st.session_state.input_method = "Upload Image"
                 ...
                 st.rerun()
        with cols[1]:
             if st.button("📸 Use Camera", key="use_camera_btn", use_container_width=True):
                 ...
                 st.rerun()
        with cols[2]:
            if st.button("🔄 Reset Scan", key="reset_btn_scan", use_container_width=True):
        ...
        # --- Camera Input Logic ---
            ...
            with st.container():
                ...
                if not st.session_state.image_captured:
                    ...
                    if st.button("📸 Capture Image", key="capture_btn", disabled=capture_btn_disabled, use_container_width=True):
                    ...
                    # if st.button("🔄 Restart Camera", key="restart_camera_btn"):
        ...
        if st.session_state.image_path and st.session_state.image_captured and st.session_state.selected_history_item is None:
            ...
            if st.button("🔍 Extract Information", key="extract_btn", type="primary", use_container_width=True):
    ...
    # --- About Tab ---
        ...
        with col_about1:
            ...
            st.markdown('<div style="font-size: 100px; text-align: center; padding-top: 20px;">🧐</div>', unsafe_allow_html=True) # Emoji
        with col_about2:
            st.markdown("""
